...
namespace Compiler
{
	namespace Intermediate
	{
		...
		{
		private:
			String GetExprString(Variable * var)	//获得表达式的字符串形式？ 这里传入的是变量，虽然名为Expr，但是没有递归，所以应该是最基本的形式.
			{
				StringBuilder sb(48);			//代表字面意义上的0，cout<<char(48) 0.
				sb.Append(L'#');
				sb.Append(String(var->Id, 16));
				return sb.ProduceString();
			}
			void GetOperandExprString(StringBuilder & sb, Operand & op)	//操作数
			{
				...
				{
					sb.Append(String(op.Values[0], 16));	//float占4个字节32位，double占8个字节64位	所以需要float的两倍，完结！
				...
				{
					sb.Append(L'#');
					sb.Append(String(op.Var->Id, 16));		//这个Id是啥？
				}
				else
					throw InvalidProgramException(L"Unknown operand type.");
			}
			String GetExprString(Instruction & instr)	//对指令的重载版本。
			{
				StringBuilder sb(128);	//对指令默认给这么大的控件.
				if (instr.Func)
				{
					sb.Append(instr.Func->Name);	//这个应该是上面那个标号？ 或者是 我觉得应该是操作符？左右操作数都有了，好吧，暂定是它.
					sb.Append(L':');				//可是没有任何地方出现了这个: 啊...
				}
				int i = 0;
				for (auto& op : instr.Operands)
				{
					if (i>0)
						sb.Append(L',');		//func跟参数之间只以 , 分割.. 那里用到这种格式了？
					GetOperandExprString(sb, op);	
				}
				return sb.ProduceString();
			}
			bool TraverseNode(ControlFlowGraph *program, ControlFlowNode * node, ScopeDictionary<String, Operand> & exprMap)	//遍历节点
			{
				bool changed = false;
				...
				{
					for (auto iter = node->Code.begin(); iter!=node->Code.end(); iter++)	//遍历当前节点里list<Instruction>里的每一个.
					{
						Instruction & instr = iter.Current->Value;	//Current 代表的是，迭代器指向的那个，在这里就是begin().所以是LinkedList<Instruction> 里的一个.
						...
						if (instr.Func == Operation::Phi)
						{
							...
							if (sameArg)	//Operands跟第一个不一样，或者... 算了先找下逻辑. (a!=b && a!=c)  的取反是 a==b || a==c ,所以这里代表的是，有参数跟a[1]一样，或者 a[0] a[0] 这种情况，这时候就？
							{
								...
								iter.Current->Delete();			//为什么呢？因为取消掉phi之后就变成了简单的赋值，不对，是“只有这一个操作数”。？嗯，左边是名字，右边是值?
							...
							{		//  a[1] = phi(a[0], a[0]) 啊.. 哦我知道了，只要有哪怕一点，两个不等于一个的情况，都应该删掉phi ? 这很奇怪，如果五个参数有三个是怎么办?
								String exprString = GetExprString(instr);//instr 是当前Node 的第一条指令.
								...
								if (exprMap.TryGetValueInCurrentScope(exprString, opValue))//opValue 是out 参数. 这里的意思是，如果此参数存在，就如此
								{
									...
									iter.Current->Delete();	//简单赋值之后，把当前指令删除掉 ...    为什么？ 不该是仅仅在phi可以被优化的时候才删掉吗?
									changed = true;
									continue;
								}
								else
