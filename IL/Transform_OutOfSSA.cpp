...
namespace Compiler
{
	...
	namespace Intermediate
	{
		...
		private:
			...
			{
				List<RefPtr<Variable>> SSA_Vars;	//是转成ssa之后的所有变量。
				List<int> SSA_VarMap; // ssa var -> coalecsed var  这里存的是数量更多的ssa var到”不干涉的都合成i一个“的规约后的var. 所以list里存的应该是， oldLiveRange->Values 里所有的 ssaVar 都对应到同一个var-Id上。 
			...
			{
				auto targetVar = new Variable(*oldVar);	//赋值一个新的变量. 传入的是什么呢？ 
				nameBuilder.Clear();
				nameBuilder<<L"^phi_"<<program->Variables.Count()<<L'_'<<targetVar->Name;	//Variable的名字是存在里面的，所以这里连name也复制过来了。
				// phi+程序里的变量个数？+ _ +此变量的名字 : ^phi_3_^tmp32_0[7]

				targetVar->Name = nameBuilder.ToString();	//并且修改成如上形式
				targetVar->Id = program->Variables.Count();	//并且此变量的id设置为新加的最后一个变量。
				program->Variables.Add(targetVar);	//只是把此变量添加到cfg的变量集合里，并且id设置为最后一个，
				program->VarDefs.Add(defNode);//defNode 是 类似“Instruction(ParallelCopy)”的东西，可是明明就是没有参数啊...
				return targetVar;
			}
			...
			void InsertCopies(ControlFlowGraph * program)	
			{
				...
				{
					for (int i = 0; i < instr.Operands.Count(); i++)	//遍历当前phi指令的所有操作数.
					{
						auto node = instr.CFG_Node->Entries[i];	//node设置为此指令在 cfg 里的前驱
						InstructionNode* instrCopy = node->Code.LastNode()->GetPrevious();//取倒数第二个指令.
			...
			TransformContext CoalescePhi(ControlFlowGraph * program)
			{
				...
				for (auto & var : program->Variables)	//对程序的每一个变量
				{
					...
					context.PhiClasses.Add(phiClass);//所有的都保存进来。
				}
				...
				context.SSA_Vars = program->Variables;//因为此刻程序的变量就是ssa形式的，所以直接赋值。 也验证了我"SSA_Vars里面存的是原初的变量"的想法.
				context.SSA_VarMap.SetSize(program->Variables.Count());		//这个map大概是为了新x'跟x映射用的? 类型是 List<int>
				return context;	
			}
			...
			{
				for (int i = 0; i<instr.Operands.Count()>>1; i++)//遍历此指令的所有参数
				{
			...
			{
				Dictionary<DataType, Variable *> tempVars;	//字典存变量与类型的对应关系。
			...
			{
				List<RefPtr<Variable>> newVars;		//重命名之后，得到的non-ssa的每个变量的值？
				...
				for (int i = 0; i<program->Variables.Count(); i++)	//遍历所有变量
				{
					...
					else //>= ParameterCount  对应非形参的变量们， 前面就直接塞入映射，是因为... 形参是跨函数的，不参与phi-webs之类的操作，就这么认为吧.
					{
						...
						if (phiClass)	//如果不为空，则
						{
							int representId = Min(phiClass->Values);		//那它min个屁啊？ 多来几次就会不一样了吗? 等等，这里的value存放的是id.
							//min求的是list里的最小值。  为什么要找最小呢. 因为 phiClass 里存的变量索引是以递增顺序进行的? 先不管.
							//作为代表整个phi-webs的存在.

							Variable * representVar = context.SSA_Vars[representId].Ptr();		//获取它在ssa_vars里存放的变量指针的值。
							...
							{
								newVar = new Variable(*representVar);	//如果找不到就搞个新的
								...
								newVars.Add(newVar);	//并且加到新名字里。
							}
							oldVarMapping[newVar] = curVar;//新旧名字互相映射. 待会会用到对吧? 
				...
				for (auto & node : program->Nodes)	//接下来是对所有节点里所有的指令进行。
				{
					for (auto instrNode = node->Code.begin(); instrNode!=node->Code.end(); ++instrNode)
					{
						...
						if (instr.Func == Operation::Phi)
						{
							...
							instrNode.Current->Delete();	//删掉的是phi所在的？Instruction，是的，仅仅删掉一条指令而已。
							continue;
						}
						//如果不是phi 而是其他指令的话，
						...
						for (auto & op : instr.Operands)//跟上面一样，区别是上面是被赋值的那个，这里是右边的一堆操作数.
							if (op.IsVariable())
								op.Var = varMapping[op.Var];	//把每个操作数也都改成新的值.
				...
				{
					auto oldVar = oldVarMapping[var.Ptr()].GetValue();	//先从map里提取出每一个旧的变量的值.
			...
			{	
				...
				for (int i = 0; i<phi1->Values.Count(); i++)	//遍历1里面所有的变量.
				{
					int var1 = context.SSA_VarMap[phi1->Values[i]];				//从ssa_var里找对应的值设为var1.
					for (int j = 0; j<phi2->Values.Count(); j++)					//然后在phi2里遍历每个var2.
					{
			...
			void OutOfSSA(ControlFlowGraph * program)
			{
				...
				/*for (auto node : program->Nodes)
				{
					...
					for (int i = 0; i<program->Variables.Count(); i++)		//原来你们是这么调试东西的，看id啊..我也这么写！
					{
