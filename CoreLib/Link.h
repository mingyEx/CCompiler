...
namespace CoreLib
{
	namespace Basic
	{
		...
		public:
			LinkedList() : FHead(nullptr), FTail(nullptr), FCount(0)
			{
			}
			~LinkedList()
			{
			...
			LinkedList(const LinkedList<T> & link) : FHead(nullptr), FTail(nullptr), FCount(0)
			{
				this->operator=(link);
			}
			LinkedList(LinkedList<T> && link) : FHead(nullptr), FTail(nullptr), FCount(0)
			{
				this->operator=(std::move(link));
			}
