...
namespace CoreLib
{
	namespace Basic
	{
		...
		private:
			inline int GetProbeOffset(int probeId) const //获取探头偏移?
			{
		...
		private:
			...
			void Free()
			{
				if (hashMap)	//nullptr为 false.
					delete[] hashMap;
				hashMap = nullptr;
			}
			inline bool IsDeleted(int pos) const
			{
				return marks.Contains((pos << 1) + 1);	//左移相当于*2对吗？是的，猜测是二叉树之类的吧..
			}
			...
			{
				if (val)
					marks.Add((pos << 1) + 1);	//这些函数到地方再说，自己想问的是，有没有stl里可以代替这些的？
				else
		...
		public:
			class Iterator	//他的迭代器是定义到自己Dictionary里面的，stl里的迭代器呢？没找到根，总之是继承来的。
			{
			private:
				const Dictionary<TKey, TValue>* dict;		//迭代器里塞自己的类，想起来那个insert_iterator哈哈哈，刚好都用上了.
				int pos;
			public:
				//这俩的区别是成员函数和非成员函数，显然。
				KeyValuePair<TKey, TValue>& operator *()	//果然是操作符重载
				{
